...
export default function RespaldoPage({ onVolver }) {
  ...
  async function handleCompartirArchivo() {
    limpiarAvisos();
    setCompartiendo(true);
    try {
      const archivo = new File([textoJson], nombreDeArchivo(), { type: 'application/json' });
      if (navigator.canShare && navigator.canShare({ files: [archivo] })) {
        await navigator.share({
          files: [archivo],
          title: 'Respaldo FUDI',
          text: 'Respaldo de datos de FUDI',
        });
        setMensaje('Respaldo compartido.');
      } else if (navigator.share) {
        // Algunos navegadores no soportan compartir archivos: se comparte el texto.
        await navigator.share({ title: 'Respaldo FUDI', text: textoJson });
        setMensaje('Respaldo compartido.');
      } else {
        setError('Este dispositivo no soporta compartir directo. Usá "Descargar respaldo".');
      }
    } catch (err) {
      if (err?.name !== 'AbortError') {
        setError('No se pudo compartir. Probá con "Descargar respaldo".');
      }
    } finally {
    ...
    <div className="page">
      ...
      <div className="page__content">
        ...
        <button
          ...
          onClick={handleCompartirArchivo}
          disabled={cargando || compartiendo}
        >
          {compartiendo ? 'Preparando…' : 'Compartir por WhatsApp o Drive'}
        </button>
        <div className="espaciador" />
        <button type="button" className="btn btn--secundario" onClick={handleDescargar} disabled={cargando}>
          Descargar respaldo
        </button>
        <div className="espaciador" />
